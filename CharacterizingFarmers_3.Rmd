...
author: 
  ...
  email: bisrat.gebrekidan@ilr.uni-bonn.de
  name: Bisrat Haile
date: "6/2/2017"
output: 
  html_document: 
...
gg<-ggplot(df2,aes(x = value)) + stat_ecdf(aes(colour = variable, geom = "step"))+
  ...
  theme(legend.title=element_blank(),plot.title = element_text(hjust = 0.5, size = 12, family = "verdana"))
gg
...
### Constraint to Intensification

```{r}
# read the intensification reason data 

#data <- as_factor(haven::read_dta("../FarmTypology/Data/IntensificationReason2.dta"), "labels")
...
plot_ly(df3,x=~value, y = ~Percent, color = ~key, colors = mycolor3) %>%
  layout(title = "Source of Information",
         xaxis = list(title = "", tickfont=list(size=13, family="verdana")),
...
df2 <- df%>%
  ...
  dplyr::summarise(count=n())%>%
  mutate(percent=(count/sum(count))*100)
#df2$Cope <- factor(df2$Cope, levels= df2$Cope[order(df2$percent, decreasing = TRUE)])
...
names(original.data2) <- c(
  ...
  "If I drop my wallet/purse somewhere within<br> this village, I am likely to get it back",
...
#set.seed(10)
#list.perceptiondata <-split(df_summary, sample(rep(1:4, 5)))
#
#for (i in 1:length(list.perceptiondata)) {

# write.csv(list.perceptiondata[i], paste0("Perception", i, ".csv"))}
...
p <- plot_ly(data, x = ~x1, y = ~y, type = 'bar', orientation = 'h', name="Strongly<br>agree",
  ...
  add_trace(x = ~x4, ame="Disagree", marker = list(color = 'rgba(164, 163, 204, 0.85)')) %>%
  add_trace(x = ~x5, ame="Strongly Disagree", marker = list(color = 'rgba(190, 192, 213, 1)')) %>%
...
The following table presents the full cleaned original data that will be used for the analysis below. 
...
```{r}
fviz_pca_var(pca1, col.var="cos2", title = "") +
...
#plot(res.hcpc2, choice ="map", draw.tree = FALSE,ind.names = FALSE, centers.plot = TRUE, title = "")
...
#foodsecurity1 <- read.csv("FoodSecu_test.csv")
...
g <- ggplot(df_box, aes(clust, FarmSize_Ha))+ stat_boxplot(geom = "errorbar", width = 0.2, linetype=3) + geom_boxplot(aes(fill = clust),
  ...
  method = "t.test",
  label = "p.signif"
) + theme_fivethirtyeight( base_family = "Verdana") + labs(
  x = "",
...
g <- ggplot(df_box, aes(clust, ShareOfRice))+ stat_boxplot(geom = "errorbar", width = 0.2, linetype=3) + geom_boxplot(aes(fill = clust),
                                                                                                                      width =
                                                                                                                        0.4, alpha = 0.5
) + stat_compare_means(comparisons = my_comparisons, label = "p.signif") + theme_fivethirtyeight( base_family = "Verdana") + labs(
  x = "",
...
fviz_dend(res.hcpc2, show_labels = FALSE, k_colors = c("#26547C","#EF476F","#FFD166"), rect = TRUE, rect_fill = TRUE, rect_border = c("#26547C","#EF476F","#FFD166"),lower_rect = -0.1, ggtheme = theme_grey(base_family = "Verdana"))
...
g <- ggplot(DataWithCluster, aes(FarmType, ShareOfRice)) + geom_boxplot(aes(fill = FarmType),
                                                                        width =
                                                                          0.4, alpha = 0.5
)+ stat_boxplot(geom = "errorbar", width = 0.2) + stat_compare_means(comparisons = my_comparisons, label = "p.signif") + theme_fivethirtyeight( base_family = "Verdana") + labs(
  x = "",
  title = "Share of land allocated to Rice by Farmer Type",
  caption = " Pairwise mean comparisons [ns: p > 0.05] [*: p <= 0.05]
  [**: p <= 0.01] [***: p <= 0.001][****: p <= 0.0001]"
...
g <- ggplot(DataWithCluster, aes(FarmType, DistanceFromIfakara)) + geom_boxplot(aes(fill = FarmType),
                                                                                width =
                                                                                  0.3, alpha = 0.8
)+ stat_boxplot(geom = "errorbar", width = 0.1) + stat_compare_means(comparisons = my_comparisons, label = "p.signif") + theme_fivethirtyeight( base_family = "Verdana") + labs(
  x = "",
